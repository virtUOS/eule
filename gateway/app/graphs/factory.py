...
One compiled graph per bot, cached. Every bot's fragment — including routers, which
compose their targets as subgraphs — is resolved by `cfg.graph` via the fragment
registry.
"""
...
def build_compiled_graph(cfg: BotCfg, checkpointer: Any, registry: Registry) -> Any:
    # Router bots are ordinary bots: `graph: "router"` resolves to the stock router
    # fragment, which composes its targets as subgraphs (docs/04 §6b, step 9c).
    guard_model = None
